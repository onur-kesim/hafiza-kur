...
**H4 çıplak ad** (6 Eyl) — dosya SADECE adıyla anınca haksız OLU basılıyordu (13 bulgunun 11'i
GÜRÜLTÜ); beyan dizinsiz + TAM BİR aday → "TAŞINMIŞ" ile AYNI kelime KULLANILMAZ ·
...
**md.9** görünürlük · **md.6-8** keşif/eşle/durma + çoklu-`canli` + hüküm ayrımı · **md.2+4+5**
README · `--kesif` **72 public depoda**, salt okuma, 0 bayt.
...
- 🔴 **ÖLÇÜM ALETİ DE YALAN SÖYLER — DOKUZ VAKA:** boru sonrası `$?` · `d[rol]=dosya` uydurdu ·
  argümanı YOK SAYAN araç · eşitlik maskelemesi · `stderr=STDOUT` md.8'i KÖR etti · **PAYLAŞILAN
  KUM HAVUZU YALANCI KIRMIZI** · **`A or B` zincirinde "boş"u tanımla** · **regex HAM BLOĞU
  okumadan hüküm vermez** · artefakt BOYUTU oracle DEĞİL. ⇒ akış BİRLEŞTİRİLMEZ · HAM ÇIKTI okunur.
- 🔴 **CI HÜKMÜ:** API URL'ine DOĞRUDAN NAVİGASYON + `JSON.parse(innerText)`; **SAYFA BAŞINA 100
  DÖNER** — `total_count` ile karşılaştır, SAYFALA (6 Eyl: 151 = 100+51). **403 ARALIKLIDIR**,
  504 de görülür ⇒ bir kez YENİLE. `WebFetch` API'de 403; tarayıcıda `fetch` ÖLÜ. 🟢 **LOG
  OKUNUR:** Chrome'da `runs/<RUN_ID>/job/<JOB_ID>` + `get_page_text` (`check_suite_id` ≠ `run_id`).
- 🔴 **CI kırmızısı KAPI kırmızısı olmayabilir** (#66: `upload-artifact` Finalize 403).
- 🔴 **KAPININ KENDİ SENARYOSU KAPIYI KIRMIZI YAKABİLİR** (md.7/A2) ⇒ gerçek vakaya çevir.
...
- 🔴 **KAPI GÖVDESİNE SATIR EKLERKEN ÇAPALARA BAKILIR — DÖRDÜNCÜ ısırık 6 Eyl** (`gitfile` 6. kol
  4→6 unutuldu; `karmasiklik` 97→99 hatırlandı). 🔴 **"TÜM `faz0` bataryası" YALNIZ `*_mutanti.py`
  DEĞİLDİR** — `yol_ayraci_kapisi` atlandı, CI #96 üç platformda kırmızı yandı. **BELGE ve YORUM
  da kapı kırar**; `yol_ayraci_kapisi` KAPI-1 YORUM ile KODU AYIRT ETMİYOR (ayrı tur).
  **ÇAPA ARACIN KENDİ ÇIKTISINDAN GÜNCELLENMEZ.** **Mesaj metni ile koşul TEK KAYNAKTAN üretilir**
  (6 Eyl: "beklenen 3" derken kod `==4` sınıyordu).
...
- 🔴 **`rmtree(ignore_errors=True)` 38 dosyada / 124 yerde, `onerror` YOK (5 Eyl, çöplenme):**
  md.8 ⇒ **KOD DEĞİŞMEZ**, ikinci ısırıkta açılır.
